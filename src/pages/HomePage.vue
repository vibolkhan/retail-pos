<template>
  <v-container class="py-6 py-md-8" fluid>
    <v-row class="mb-6" align="center">
      <v-col cols="12" md="7">
        <v-chip class="mb-4" color="primary" prepend-icon="mdi-store" variant="tonal">
          Retail POS System
        </v-chip>

        <h1 class="text-h4 text-md-h3 font-weight-bold mb-3">Welcome back, Cashier</h1>

        <p class="text-body-1 text-medium-emphasis mb-5">
          Manage sales, products, and orders from a clean mock-data dashboard.
        </p>

        <v-btn color="primary" prepend-icon="mdi-cash-register" size="large" to="/pos" variant="flat">
          Start Selling
        </v-btn>
      </v-col>
      ...
      <v-col cols="12" md="5">
        <!-- Today Overview Card -->
        <v-card class="pa-5" rounded="lg" variant="flat" color="primary">
          ...
          <div class="text-body-2 opacity-80 mb-2">
            ...
            <v-chip class="ma-1" color="success" variant="tonal" size="small">
              Revenue: {{ formatCurrency(todaySales.reduce((t, s) => t + s.grandTotal, 0)) }}
            </v-chip>
          </div>
        </v-card>
      </v-col>
    ...
    <v-row>
      <v-col cols="12" md="3" sm="6" v-for="card in dashboardCards" :key="card.title" class="mb-4">
        <v-card class="h-100 pa-5" rounded="lg">
...
<script lang="ts" setup>
  ...
  const todaySales = computed(() => {
    ...
    return sales.value.filter(sale => new Date(sale.date).toDateString() === today)
  })
  ...
  const dashboardCards = computed(() => [
    ...
    {
      title: 'Total Revenue',
      value: formatCurrency(sales.value.reduce((total, sale) => total + sale.grandTotal, 0)),
