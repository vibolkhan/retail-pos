    ...
    <div class="d-flex flex-column flex-md-row align-md-center justify-space-between ga-4 mb-5">
      <div>
        ...
        <p class="text-body-2 text-medium-emphasis mb-0">
          Search products, filter by category, and add available stock to the cart.
        </p>
      </div>
  ...
  const filteredProducts = computed(() => {
    ...
    return products.value.filter(product => {
      ...
      const matchesSearch = query
        ? [product.name, product.code, product.barcode].some(value => value.toLowerCase().includes(query))
        : true
